...
def run_cmd(cmd: List[str], cwd: Optional[Path] = None, timeout: int = 90) -> CommandResult:
    proc = subprocess.run(
        cmd,
...
def _legacy_script_path() -> Optional[Path]:
...
def choose_option(title: str, items: Sequence[Dict]) -> Optional[Dict]:
...
def _pick_dbconsult() -> Optional[Path]:
...
def option_generic(label: str, command: Optional[List[str]] = None) -> None:
    append_usage(label)
    if not command:
